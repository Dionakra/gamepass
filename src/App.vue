      ...
      <div class="w-full md:w-2/3">
        <div class="flex flex-wrap mb-3">
          ...
          <select
            ...
            class="w-full md:w-1/4 px-2 py-1 border border-gray-300 rounded-md"
            :class="selectedCategory == '' ? 'text-gray-500' : 'text-gray-900'"
          >
        ...
        <div class="flex flex-wrap md:flex-inline justify-around">
          ...
          <div class="mb-2 md:mb-0">
            <div class="flex flex-inline justify-center">
              <button
                aria-label="Filter by PC game"
                class="mx-2 text-white font-bold w-12 rounded-lg p-2 h-12"
                @click="addPlatform('PC')"
                :class="platforms.includes('PC') ? 'bg-xbox' : 'bg-gris'"
              >
                <IconPC />
              </button>

              <button
                aria-label="Filter by Console game"
                class="mx-2 text-white font-bold w-12 rounded-lg p-2 h-12"
                @click="addPlatform('XBOX')"
                :class="platforms.includes('XBOX') ? 'bg-xbox' : 'bg-gris'"
              >
                <IconXbox />
              </button>

              <button
                aria-label="Filter by Cloud game"
                class="mx-2 text-white font-bold w-12 rounded-lg p-2 h-12"
                @click="addPlatform('XCLOUD')"
                :class="platforms.includes('XCLOUD') ? 'bg-xbox' : 'bg-gris'"
              >
                <IconXCloud />
              </button>
            </div>
          ...
          <div class="mb-2 md:mb-0 space-y-2">
            <div class="flex flex-inline justify-center">
              ...
              <button
                ...
                @click="localCoop = !localCoop"
                :class="localCoop ? 'bg-xbox' : 'bg-gris'"
                title="Local Cooperative"
              >
                <IconCoop />
              </button>
              ...
              <a class="cursor-pointer" target="_blank" rel="noopener" :href="'https://www.microsoft.com/en-us/store/p/' + game.linkTitle + '/' + game.id">
                ...
                <div class="border-l border-r">
                  <div class="px-2 mb-2">
                    ...
                    <div class="text-white font-semibold flex text-xs -mx-1.5 justify-center">
                      <div class="w-1/2">
                        <div v-if="game.localCoop || game.localMultiplayer" class="mt-1">
                          ...
                          <div class="flex space-x-0.5">
                            <div v-if="game.localCoop" class="flex bg-xbox pl-1 pr-2 py-1 h-8 rounded-full w-max" title="Local Cooperative">
                              <div class="h-4 my-auto">
                                <IconCoop />
                              </div>
                              <div class="my-auto pl-1" v-if="game.localCoop.min && game.localCoop.max">{{ game.localCoop.min }}-{{ game.localCoop.max }}</div>
                            </div>

                            <div v-if="game.localMultiplayer" class="flex bg-xbox pl-1 pr-2 py-1 h-8 rounded-full w-max" title="Local Multiplayer">
                              <div class="h-4 my-auto">
                                <IconMultiplayer />
                              </div>
                      ...
                      <div class="w-1/2 flex justify-end space-x-0.5">
                        <div v-if="game.localCoop || game.localMultiplayer" class="mt-1">
                          ...
                          <div class="flex space-x-0.5">
                            <div v-if="game.onlineCoop" class="flex bg-xbox pl-1 pr-2 py-1 h-8 rounded-full w-max" title="Online Cooperative">
                              <div class="h-4 my-auto">
                                <IconCoop />
                              </div>
